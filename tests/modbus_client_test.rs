...
async fn test_modbus_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test 1: Read temperature register
    let temp_raw = ctx.read_holding_registers(40001, 1).await??;  // Double ? to unwrap Result<Result<...>>
    ...
    // Test 2: Read heater state
    let heater_state = ctx.read_holding_registers(40002, 1).await??;
    ...
    let temp_raw_new = ctx.read_holding_registers(40001, 1).await??;
    ...
    // Test 8: Read both registers at once
    let both = ctx.read_holding_registers(40001, 2).await??;
